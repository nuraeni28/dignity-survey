...
@section('title', 'Interviews Relawan Sosmed')
    ...
    <div class="col">
        <div style="display: flex; align-items: center;">
            <div class="d-flex justify-content-between"
                ...
                <form action="{{ route('interview-sosmed.index') }}" method="GET" class="form-inline d-flex">
        ...
        <div class="col" style="margin-top:20px">
            <form action="{{ route('interview-sosmed.index') }}" method="GET">
            ...
            <table class="table">
                <thead>
                    <tr>
                        ...
                        <th style="width: 5% !important;">{{ __('Photo') }}</th>

                        <th> {{ __('Interview Date') }}</th>
                        <th style="width: 50% !important;">{{ __('Location') }}</th>
                        <th>{{ __('Relawan') }}</th>
                        <th>{{ __('Responden') }}</th>
                        <th>{{ __('Duration') }}</th>
                        <th>{{ __('File Rekaman') }}</th>
                        <th>{{ __('Jenis') }}</th>
                        <th> {{ __('Aksi') }}</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach ($interviews as $interview)
                        <tr>
                            <td><input type="checkbox" name="ids[]" value="{{ $interview->id }}"
                                    class="checkbox_ids">
                            </td>
                            <td>
                                <a href="{{ url('public//public/image/' . $interview->interview->photo) }}"
                                    target="_blank">
                                    <img src="{{ url('public//public/image/' . $interview->interview->photo) }}"
                            ...
                            <td>
                                @if ($interview->interview == null)
                                    -
                                @else
                                    {{ date('d/m/Y', strtotime($interview->interview->interview_date)) }}
                                @endif
                            </td>
                            <td>
                                @if ($interview->interview == null)
                            ...
                            <td>
                                {{ \Carbon\Carbon::parse($interview->interview->start_time)->diffInMinutes(\Carbon\Carbon::parse($interview->interview->end_time)) }}
                                Menit
                            </td>
                            <td>
                                @if ($interview->interview->record_file)
                                    <a href="{{ url('public/public/record/' . $interview->interview->record_file) }}"
                                        target="blank">{{ $interview->interview->record_file }}</a>
                                @else
                                    -
                                @endif
                            </td>
                            <td>
                                @if (pathinfo($interview->interview->record_file, PATHINFO_EXTENSION) === 'mp3')
                                    Offline
                                @else
                                    Online
                                @endif
                            ...
                            <td>
                                <nobr class="d-flex">
                                    @canany(['owner', 'admin', 'super-admin'])
                                        <a href="{{ route('interview-sosmed.edit', $interview->id) }}"
                                            class="btn btn-xs btn-default text-primary mx-1 shadow custom-button"
                                            title="Edit">
                                            <i class="fa fa-lg fa-fw fa-pen"></i>
                                        </a>
                                        <form class="flex" action="{{ route('interview-sosmed.destroy', ['interview_sosmed' => $interview->id, 'page' => $interviews->currentPage()]) }}"
                                            method="POST">
                                            @csrf
                                            @method('DELETE')
                                    ...
                                    @endcan
                                    <a href="{{ route('interview-sosmed.show', $interview->id) }}"
